...
N=32M floats, best of 5 runs. See [loop_unswitch.md](loop_unswitch.md) for the Apple M5 baseline
...
This is a much smaller gap than the M5 doc's 2.00x–3.25x. `objdump` explains why: **at `-O2` on
...
per element, not a scalar-vs-SIMD difference — the mechanism the M5 doc's "why it matters" section
is actually about.
...
the opposite of the M5 story, where the gap is largest exactly at `-O2` (2.00x–3.25x) because that's
...
`-O` level on this GCC/x86 build that reproduces the M5 doc's headline scalar-vs-SIMD comparison.

---

## Comparison to Apple M5

| mode | M5 switched | M5 unswitched | M5 speedup | Ryzen -O2 switched | Ryzen -O2 unswitched | Ryzen -O2 speedup |
|---|---:|---:|---:|---:|---:|---:|
...
really about branches.** On M5, Apple Clang at `-O2` vectorizes the unswitched loop with NEON
...
**The "why it matters: vectorization" framing in the M5 doc does not hold on this build at `-O2`.**
...
with the M5 doc's own note that `-O3` closes the gap by unswitching automatically. In other words:
Ryzen at `-O2` behaves like a slowed-down version of what the M5 doc calls the `-O3` outcome —
scalar-vs-scalar with only the branch bookkeeping as overhead — rather than the `-O2` outcome the
M5 doc describes (scalar-vs-SIMD).

**Absolute vectorized throughput is also lower here.** At `-O3`, both loops converge to 16 ms
(384 MB / 16 ms ≈ 24 GB/s effective single-thread bandwidth) vs M5's 4 ms convergence (≈96 GB/s).
This gap is expected: M5's doc figure reflects Apple Silicon's unified, very high single-core
...
1. **The compiler-specific vectorization threshold matters more than the ISA here.** Whether the
   ...
   actually vectorizes at the optimization level being tested — true for Clang at `-O2` on M5, false
